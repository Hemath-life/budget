...
export async function POST(request: NextRequest) {
  try {
    ...
    const stmt = db.prepare(`
      INSERT INTO transactions (id, type, amount, currency, category, description, date, is_recurring, recurring_id, tags, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    ...
    if (body.type === 'expense') {
      const budget = db.prepare('SELECT * FROM budgets WHERE category = ?').get(body.category) as Record<string, unknown> | undefined;
      if (budget) {
