...
export const BASE_THEMES = AVAILABLE_THEMES.filter((theme) =>
  ['light', 'dark', 'system'].includes(theme.value),
);

export const SHADCN_THEMES = AVAILABLE_THEMES.filter((theme) =>
  ['stone', 'gray', 'slate'].includes(theme.value),
);

export const MONDAY_THEMES = AVAILABLE_THEMES.filter((theme) =>
  ['monday'].includes(theme.value),
);
